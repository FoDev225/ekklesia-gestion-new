<div>
    ...
    <input type="text" name="name" id="name"
           ...
           value="{{ old('name', $worshipGroup->name ?? '') }}" required>
    @error('name')
...
<div>
    ...
    <textarea name="description" id="description" rows="4"
              class="w-full border-gray-300 rounded-md text-sm focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $worshipGroup->description ?? '') }}</textarea>
    @error('description')
        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
    @enderror
</div>
    ...
    <select name="leader_id" id="leader_id"
        ...
        @foreach ($believers as $believer)
            <option value="{{ $believer->id }}"
                @selected(old('leader_id', $worshipGroup->leader_id ?? null) == $believer->id)>
