...
@section('doc-title', 'LISTE DES MEMBRES - ' . strtoupper($team->name))
...
<div class="intro-box">
    Liste nominative des membres inscrits au sein de l'équipe
    <span class="highlight">{{ $team->name }}</span>
    de l'<span class="highlight">Eglise Locale AEBECI {{ $church->church_name }}</span>,
    comptant <span class="highlight">{{ $team->believers->count() }} membre(s)</span> à ce jour.
    @if($team->leader)
        Responsable de l'équipe : <span class="highlight">{{ $team->leader->full_name ?? $team->leader->name }}</span>.
    @endif
</div>
