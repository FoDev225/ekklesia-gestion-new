...
@section('doc-title', 'PROGRAMME DES CULTES DE L\'ÉGLISE BAPTISTE AEBECI DE YOPOUGON NOUVEAU BUREAU')
...
<div class="prog-header">
    ...
    <div class="theme">THÈME : {{ strtoupper($periode->general_theme) }}</div>
    @endif
    <div class="periode-info">
        PÉRIODE : {{ strtoupper($periode->name) }} &nbsp;|&nbsp;
        Du {{ $periode->start_date?->format('d/m/Y') }} au {{ $periode->end_date?->format('d/m/Y') }}
    </div>
</div>
...
<table class="programme">
    ...
    <tbody>
        @forelse($periode->services as $service)
        @php
        ...
        <tr>
            {{-- Date --}}
            <td class="date-cell">
                {{ $service->service_date?->translatedFormat('d M') }}
            </td>

            {{-- Thème --}}
            <td>
                <span class="theme-text">{{ $service->service_theme ?? '—' }}</span>
            </td>

            {{-- Prédicateur titulaire --}}
            <td>
                @if($predTit)
                    <span class="person-main">{{ $predTit->believer->lastname }}</span>
                    <span class="person-main" style="font-weight:normal">{{ $predTit->believer->firstname }}</span>
                @else
                    <span class="no-assign">—</span>
                @endif
            </td>

            {{-- Prédicateur suppléant --}}
            <td>
                @if($predSup)
                    <span class="person-backup">{{ $predSup->believer->full_name }}</span>
                @else
                    <span class="no-assign">—</span>
                @endif
            </td>
            ...
            <td>
                @if($presTit)
                    <span class="person-main">{{ $presTit->believer->lastname }}</span>
                    <span class="person-main" style="font-weight:normal">{{ $presTit->believer->firstname }}</span>
                @else
                    <span class="no-assign">—</span>
                @endif
            </td>
            ...
            <td>
                @if($presSup)
                    <span class="person-backup">{{ $presSup->believer->full_name }}</span>
                @else
                    <span class="no-assign">—</span>
                @endif
            </td>

            {{-- Louange (2 ou 3 groupes) --}}
            <td>
                @forelse($louangeGroups as $i => $group)
                    <span class="group-name" style="color:{{ $groupColors[$i % count($groupColors)] }}">
                        {{ $group->name }}
                    </span>
                @empty
                    <span class="no-assign">—</span>
            ...
            <td>
                @if($annonceur)
                    <span class="person-main">{{ $annonceur->believer->full_name }}</span>
                @else
                    <span class="no-assign">—</span>
                @endif
            </td>
...
<div class="legend">
    ...
    <div class="legend-item">{{ $periode->services->count() }} cultes programmés</div>
</div>
