...
@section('title', 'Réunions du comité')
@section('page-title', 'Réunions du comité')
...
<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            <a href="{{ route('comite.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Comité</a>
            <span class="text-gray-300">/</span>
            <span class="text-sm text-gray-700 font-medium">Réunions</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="md:col-span-2">
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 font-medium text-gray-900">{{ $meeting->meeting_date->format('d/m/Y') }}</td>
                                <td class="px-4 py-3 font-medium text-gray-900">{{ $meeting->meeting_time?->format('H:i') }}</td>
                                <td class="px-4 py-3">
                                    <span class="px-2 py-0.5 text-xs font-medium rounded
                                        {{ $meeting->meeting_type === 'extraordinaire' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700' }}">
                                        {{ $meeting->type_label }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-gray-600 text-xs max-w-sm truncate">{{ $meeting->meeting_objective }}</td>
                                <td class="px-4 py-3 text-center">
                                    @if($meeting->rapport_path)
                                        <a href="{{ $meeting->rapport_url }}" target="_blank" class="text-xs px-2 py-1 bg-cyan-100 text-cyan-700 rounded">Voir</a>
                                    @else
                ...
                <div class="px-4 py-3 border-t border-gray-200">{{ $meetings->links() }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="md:col-span-1">
            <div class="bg-white shadow-sm rounded-lg p-4">
                <h3 class="text-sm font-semibold text-gray-700 uppercase mb-3">Nouvelle réunion</h3>
                <form action="{{ route('comite.meetings.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                    @csrf
                    <div>
                        <label class="block text-xs font-medium text-gray-500 uppercase mb-1">Type <span class="text-red-500">*</span></label>
                        <select name="meeting_type" required class="w-full border-gray-300 rounded-md text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="meeting_date" required value="{{ old('meeting_date', now()->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <textarea name="meeting_objective" rows="4" required
                            class="w-full border-gray-300 rounded-md text-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('meeting_objective') }}</textarea>
                    </div>
                    <div>
